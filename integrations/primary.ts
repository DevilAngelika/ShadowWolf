...
export async function sendButtons(client: Client) {
  ...
  const otherTicketing: ButtonBuilder = createButton(
    'other',
    'Autres demandes',
    ButtonStyle.Secondary
  );
  const suggestTicketing: ButtonBuilder = createButton(
    'suggest',
    'Suggestion pour le serveur',
    ButtonStyle.Secondary
  );
  ...
  const embedTicketing = createEmbed(
    'Système de Tickets',
